...
export class ProductService {
  ...
  getProducts(limit: number, catId: number = 0, subCatId: number = 0, relatedId = 0, query = ''): Observable<Product[]> {
    return this.http.get<Product[]>(
      `${environment.apiUrl}products?limit=${limit}&catId=${catId}&subCatId=${subCatId}&relatedId=${relatedId}&query=${query}`);
  }
